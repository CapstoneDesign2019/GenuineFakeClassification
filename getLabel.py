...
#    image_path, appkey = sys.argv[1], sys.argv[2]
    appkey='9f69bf7a62e57783e3aa9adef4aa7f5e'
    src = cv2.imread(image_path,cv2.IMREAD_COLOR)
    src = cv2.resize(src,dsize=(1024,1024),interpolation=cv2.INTER_AREA)
...
#    sorted(output['result']['boxes'])
    ...
    print(y_c-100,y_c+100,x_c-130,x_c+130)
    src = cv2.imread(image_path,cv2.IMREAD_COLOR)
    #cv2.imwrite(image_path,src[y_s-100:y_e+50, x_s-30:x_e+30])
    cv2.imwrite(image_path,src[y_c-100:y_c+50, x_c-75:x_c+75])
    # y 세로 x가로 +면 밑으로, 오른쪽으로
    #roi = src[]
    #[[547, 399], [645, 394], [646, 407], [548, 412]]
...
if __name__ == "__main__":
    for root, dirs, files in os.walk('/home/lgc/Desktop/newData/test'):
        for fname in files:
            main(root+"/"+fname)
